...
class FolderItem extends FileItem implements MapModel {
  ...
  void draw() {
    ...
    if (parent == zoomItem) {
      ...
      myFolder = loadImage("folder.png");
     
      //image(myFolder,((boxWidth)/2)-((boxWidth)/6),((boxHeight)/2)-((boxHeight)/6),(boxWidth)/3,(boxHeight)/3);

        image(myFolder,boxLeft+(boxWidth/4),boxTop+ (boxHeight/4),(boxRight-boxLeft)/2,(boxRight-boxLeft)/2);
    }   //(boxBottom-boxTop)/2
  }
  ...
  void drawTag() {
    ...
    } else if (boxBottom + boxHeight < height-50) {
